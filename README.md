...
EarTap 将 macOS 发来的播放（`play`）、暂停（`pause`）和播放/暂停切换（`toggle`）命令映射为当前应用中的同一个快捷键。适合触发输入法语音输入，或目标应用支持的其他快捷操作。

> **状态：0.1.5，实验性。** 系统决定哪个应用接收媒体命令。EarTap 不保证每次实体按键都能送达，也不能确认目标应用是否执行了快捷键。
...
- 菜单栏图标直接绘制为白色，不由 macOS 模板系统重新着色；通过图形变化和暂停透明度显示就绪、暂停、发送中和错误/需授权状态。
- 不录音、不识别文字、不读取目标应用的录音状态、不联网。
- 不实现“第一次开始、第二次结束”；`play` / `pause` / `toggle` 命令都发送相同配置。
- 不映射音量键，也不是全局键盘重映射工具。
...
3. 按耳机播放键；收到的播放、暂停和切换命令映射为同一个快捷键。
...
| 现象 | 检查方向 |
| --- | --- |
| 实体按键后“收到”没有增加 | 媒体命令可能未送达 EarTap，检查系统媒体路由与映射开关 |
