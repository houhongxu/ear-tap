...
EarTap 接收 macOS 发来的播放、暂停和播放/暂停切换命令，在当前应用中发送同一个快捷键。适合触发输入法语音输入，或目标应用支持的其他快捷操作。

> **状态：0.1.0，实验性。** 系统决定哪个应用接收媒体命令。EarTap 不保证每次实体按键都能送达，也不能确认目标应用是否执行了快捷键。
...
- 不实现“第一次开始、第二次结束”；每条收到的媒体命令都发送相同配置。
- 不映射音量键，也不是全局键盘重映射工具。
...
3. 按耳机播放键；收到的播放、暂停、切换命令都映射为同一个快捷键。
...
| 现象 | 检查方向 |
| --- | --- |
| 实体按键后“收到”没有增加 | 命令未送达 EarTap，可能由其他播放器接收或系统没有分发 |
